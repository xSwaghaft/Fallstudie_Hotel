...
public class BookingManagementView extends VerticalLayout {
    ...
    // Check-in/out logic (restored with refresh of both grids)
    ...
    /**
     * Opens a dialog asking if payment was made with cash. If yes, creates payment & invoice, then checks in.
     */
    private void openPaymentDialog(Booking booking) {
        ...
        Button yesButton = new Button("Yes, Create Invoice", e -> {
            try {
                ...
                Notification.show(ex.getMessage() != null ? ex.getMessage() : "Payment/invoice creation failed", 5000, Notification.Position.MIDDLE);
            }
        });
        ...
        dialog.add(new VerticalLayout(message, new HorizontalLayout(yesButton, cancelButton)));
        dialog.open();
    }
